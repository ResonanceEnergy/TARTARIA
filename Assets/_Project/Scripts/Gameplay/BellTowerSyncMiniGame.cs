...
using Tartaria.Core;
using Tartaria.Audio; // DEBT #24: real audio wiring via AudioManager.Instance.PlaySFX2D
...
namespace Tartaria.Gameplay
{
    ...
    public class BellTowerSyncMiniGame : MonoBehaviour
    {
        ...
        IEnumerator PlaySequenceDemonstration()
        {
            ...
            foreach (int bellIndex in _targetSequence)
            {
                if (bellIndex < bellButtons.Count)
                {
                    ...
                    // DEBT #24: AudioManager.PlaySFX2D(string) — AudioManager.cs:339
                    AudioManager.Instance?.PlaySFX2D($"BellTone_{bellIndex}");
        ...
        public void OnBellPressed(int bellIndex)
        {
            ...
            HighlightBell(bellIndex);
            // DEBT #24: player-tap feedback through AudioManager.
            AudioManager.Instance?.PlaySFX2D($"BellTone_{bellIndex}");
        ...
        void SucceedGame()
        {
            ...
            // DEBT #24: success stinger via AudioManager (AudioManager.cs:339).
            AudioManager.Instance?.PlaySFX2D("BellTowerSuccess");
        ...
        void FailGame()
        {
            ...
            // DEBT #24: fail stinger via AudioManager.
            AudioManager.Instance?.PlaySFX2D("BellTowerFail");
