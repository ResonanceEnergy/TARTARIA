...
namespace Tartaria.Gameplay
{
    ...
    public class PlayerHealthController : MonoBehaviour, ISaveDataProvider
    {
        ...
        PlayerAbilityController _abilities; // DEBT #20: cached sibling for shield mitigation
        ...
        void Awake()
        {
            ...
            // DEBT #20: wire shield mitigation to sibling PlayerAbilityController.
        ...
        public void TakeDamage(float amount, GameObject instigator = null)
        {
            ...
            // DEBT #20: Frequency Shield mitigation — PlayerAbilityController.ShieldActive
            // (PlayerAbilityController.cs:39) halves incoming damage while the shield is up.
            if (_abilities == null) _abilities = GetComponent<PlayerAbilityController>();
            if (_abilities != null && _abilities.ShieldActive)
            {
                float mitigated = amount * 0.5f;
                Debug.Log($"[PlayerHealth] Frequency Shield active — mitigated {amount:F1} → {mitigated:F1}");
                amount = mitigated;
            }
        ...
        /// DEBT #20: real implementation — flips IsInvulnerable and parks the timer
        /// far in the future when ON, releases it immediately when OFF. TakeDamage()
        /// already early-outs on IsInvulnerable (see line 109).
        /// </summary>
        public void SetInvulnerable(bool value)
        {
            ...
            if (value)
            {
                // Park end-time far in the future so Update()'s automatic release
                // (line 98) cannot turn it off while the cutscene/giant-mode is active.
                _invulnerabilityEndTime = Time.time + 9999f;
            }
            else
            {
                _invulnerabilityEndTime = Time.time;
            }
