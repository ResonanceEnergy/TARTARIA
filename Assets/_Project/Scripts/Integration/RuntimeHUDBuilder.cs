...
namespace Tartaria.Integration
{
    ...
    public class RuntimeHUDBuilder : MonoBehaviour
    {
        ...
        static void Bootstrap()
        {
            ...
            // Wave 6 (2026-06-04) — prefab-first path. Per CLAUDE.md anti-circling mandate:
            // a baked HUD_Root.prefab at Assets/_Project/Resources/Prefabs/UI/HUD_Root.prefab
            // is loaded + instantiated instead of fabricating 64 GameObjects at runtime.
            // If the prefab exists, we instantiate it under a host GameObject and the
            // baked SerializeField references on the prefab's HUDController already point
            // at the prefab children — no reflection wiring needed. Controllers self-bootstrap
            // via their own Awake (HUDController sets Instance, QuestLogUI, WorldMapUI, etc.).
            // Fall back to the original fabrication path if the asset is absent.
            var prefab = Resources.Load<GameObject>("Prefabs/UI/HUD_Root");
            if (prefab != null)
            {
                var host = new GameObject("RuntimeHUDBuilder");
                DontDestroyOnLoad(host);
                var hudInstance = Object.Instantiate(prefab, host.transform);
                hudInstance.name = "HUD_Canvas"; // strip "(Clone)" suffix for hierarchy hygiene
                Debug.Log("[RuntimeHUDBuilder] Loaded HUD_Root.prefab from Resources — skipping runtime fabrication (Wave 6 prefab-first path).");
                // Still attach the builder MonoBehaviour so OnDestroy/Update keyboard handlers fire
                // and event subscriptions (pause toggle, aether vision, building restored, live RS/Aether)
                // are wired against the prefab-instantiated UI references.
                var builder = host.AddComponent<RuntimeHUDBuilder>();
                builder.AdoptInstantiatedPrefab(hudInstance);
                return;
            }
        ...
        void Awake()
        {
            ...
            // If AdoptInstantiatedPrefab() set _canvasRT during the prefab-first path,
            // skip runtime fabrication. SubscribeLiveData + event hooks still run below.
            if (_canvasRT != null)
            {
                ...
                SubscribeLiveData();
                // Re-cache live-data TMP refs from the instantiated prefab hierarchy so
                // OnRSChanged / OnAetherEnergyChanged subscribers update the visible text.
                RecacheLiveDataRefsFromPrefab();
                return;
            }
        ...
        /// <summary>
        /// Wave 6 — invoked by Bootstrap when the prefab-first path succeeds. Sets _canvasRT
        /// so Awake() short-circuits the runtime fabrication block and the controller-side
        /// SerializeField wiring stored in the prefab is used as-is. Called BEFORE Awake.
        /// </summary>
        internal void AdoptInstantiatedPrefab(GameObject hudInstance)
        {
            if (hudInstance == null) return;
            _canvasRT = hudInstance.GetComponent<RectTransform>();
            if (_canvasRT == null) _canvasRT = hudInstance.GetComponentInChildren<RectTransform>();
            _hudWired = true; // SerializeFields baked into the prefab — no reflection wire-up required
        }

        /// <summary>
        /// Wave 6 — when the prefab-first path runs, re-find the two TMP_Text refs we cache
        /// in fields so OnRSChanged / OnAetherEnergyChanged still mutate visible text after
        /// AdoptInstantiatedPrefab() bypassed WireHUD(). Lookup is name-based against the
        /// known hierarchy created in WireHUD(): RSGauge/RSText and AetherBar/AetherText.
        /// </summary>
        void RecacheLiveDataRefsFromPrefab()
        {
            if (_canvasRT == null) return;
            var rsGo = FindDescendantByName(_canvasRT.transform, "RSText");
            if (rsGo != null) _rsText = rsGo.GetComponent<TextMeshProUGUI>();
            var aetherGo = FindDescendantByName(_canvasRT.transform, "AetherText");
            if (aetherGo != null) _aetherText = aetherGo.GetComponent<TextMeshProUGUI>();
            var rsDisplayGo = FindDescendantByName(_canvasRT.transform, "RSDisplayText");
            if (rsDisplayGo != null) _rsDisplayText = rsDisplayGo.GetComponent<TextMeshProUGUI>();
            // Cache pause-menu + tuning overlay refs so HandleTogglePause / T-key still work.
            var pauseGo = FindDescendantByName(_canvasRT.transform, "PauseMenu");
            if (pauseGo != null) _pauseMenuPanel = pauseGo.gameObject;
            var tuneGo = FindDescendantByName(_canvasRT.transform, "TuningOverlay");
            if (tuneGo != null) _tuningOverlayGO = tuneGo.gameObject;
        }
        ...
        void WireHUD(HUDController hud, RectTransform canvasRT)
        {
            ...
            // AddComponent<TextMeshProUGUI> on a GameObject lacking RectTransform throws — TMP requires RectTransform. Spawn a child GO with RectTransform first, then add TMP.
