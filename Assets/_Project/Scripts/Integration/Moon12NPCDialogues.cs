...
#pragma warning disable CS0414 // Placeholder counts for planned features
{
    ...
    public class Moon12NPCDialogues : MonoBehaviour
    {
        [Header("Moon 12: Shadow NPCs")]
        ...
        void SpawnNPCs()
        {
            for (int i = 0; i < questGiverCount; i++)
                CreateQuestGiver($"QuestGiver_{i}", new Vector3(Random.Range(-50f, 50f), 0.5f, Random.Range(-50f, 50f)), $"moon12_quest_{i}", new string[] { "Greetings, adventurer.", "I have a task for you." });
            for (int i = 0; i < merchantCount; i++)
                CreateMerchant($"Merchant_{i}", new Vector3(Random.Range(-50f, 50f), 0.5f, Random.Range(-50f, 50f)), $"shop12_{i}", new string[] { "Looking to trade?", "I have rare goods." });
            ...
            CreateHelper("Helper_Shadow", new Vector3(0f, 0.5f, 0f), new string[] { "Need help? Follow the markers.", "Check your map." });
            Debug.Log($"💬 Moon12NPCDialogues: {npcs.Count} NPCs spawned");
        }
        GameObject CreateQuestGiver(string name, Vector3 pos, string questId, string[] dialogue) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Capsule); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(1f, 2f, 1f); var npc = obj.AddComponent<Moon12DialogueNPC>(); npc.npcName = name; npc.npcType = "questGiver"; npc.dialogueLines = dialogue; npc.questId = questId; npcs.Add(obj); return obj; }
        GameObject CreateMerchant(string name, Vector3 pos, string shopId, string[] dialogue) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Capsule); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(1f, 2f, 1f); var npc = obj.AddComponent<Moon12DialogueNPC>(); npc.npcName = name; npc.npcType = "merchant"; npc.dialogueLines = dialogue; npc.shopId = shopId; npcs.Add(obj); return obj; }
        GameObject CreateLoreNPC(string name, Vector3 pos, string[] loreDialogue) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Capsule); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(1f, 2f, 1f); var npc = obj.AddComponent<Moon12DialogueNPC>(); npc.npcName = name; npc.npcType = "lore"; npc.dialogueLines = loreDialogue; npcs.Add(obj); return obj; }
        GameObject CreateHelper(string name, Vector3 pos, string[] hints) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Capsule); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(1f, 2f, 1f); var npc = obj.AddComponent<Moon12DialogueNPC>(); npc.npcName = name; npc.npcType = "helper"; npc.dialogueLines = hints; npcs.Add(obj); return obj; }
        void OnDestroy() { foreach (var npc in npcs) if (npc != null) Destroy(npc); npcs.Clear(); }
    }
    public class Moon12DialogueNPC : MonoBehaviour, IInteractable
    {
