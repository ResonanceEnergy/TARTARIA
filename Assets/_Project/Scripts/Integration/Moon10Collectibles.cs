...
namespace Tartaria.Integration
{
    ...
    public class Moon10Collectibles : MonoBehaviour
    {
        [Header("Moon 10: Time Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon10_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(0.8f, 0.6f, 1f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"TemporalRelic_Moon10_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "TemporalRelic", 15f, new Color(0.7f, 0.5f, 0.9f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon10_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(0.7f, 0.5f, 0.9f));
            Debug.Log($"💎 Moon10Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon10_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Time Aether Fragment", "TemporalRelic" => "Ancient Time Relic", "HiddenCache" => "Hidden ChronoCache", _ => "Unknown Item" };
