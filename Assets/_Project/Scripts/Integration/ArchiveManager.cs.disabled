...
namespace Tartaria.Integration
{
    ...
    public class ArchiveManager : MonoBehaviour, ISaveDataProvider
    {
        ...
        void Awake()
        {
            ...
            // ISaveDataProvider registration
        ...
        public string GetProviderKey() => "ArchiveManager";

        public object GetSaveData()
        {
            return new ArchiveManagerSaveData
            {
                unlockedEntryIds = GetUnlockedIds(),
                cumulativeRS = _cumulativeRS,
                rsTier25 = _rsTier25,
                rsTier50 = _rsTier50,
                rsTier75 = _rsTier75
            };
        ...
        public void RestoreSaveData(object data)
        {
            if (data is string json)
            {
                try
                {
                    var saveData = JsonUtility.FromJson<ArchiveManagerSaveData>(json);
                    if (saveData != null)
                    {
                        RestoreFromSave(saveData.unlockedEntryIds);
                        _cumulativeRS = saveData.cumulativeRS;
                        _rsTier25 = saveData.rsTier25;
                        _rsTier50 = saveData.rsTier50;
                        _rsTier75 = saveData.rsTier75;

                        // Push unlocked state to UI
                        ArchiveUI.Instance?.SetUnlockedIds(_unlocked);

                        Debug.Log($"[ArchiveManager] Restored {saveData.unlockedEntryIds?.Length ?? 0} unlocked entries, RS {saveData.cumulativeRS:F1}");
                    }
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"[ArchiveManager] Failed to restore save data: {e.Message}");
                }
            }
            else
            {
                Debug.LogWarning("[ArchiveManager] RestoreSaveData received invalid data type");
            }
        }
    }

    /// <summary>
    /// Serializable save data for ArchiveManager ISaveDataProvider.
    /// Must be JSON-serializable (no MonoBehaviour, no Unity objects).
    /// </summary>
    [System.Serializable]
    public class ArchiveManagerSaveData
    {
        public string[] unlockedEntryIds;
        public float cumulativeRS;
        public bool rsTier25;
        public bool rsTier50;
        public bool rsTier75;
    }
}
