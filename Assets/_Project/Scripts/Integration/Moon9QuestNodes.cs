...
namespace Tartaria.Integration
{
    ...
    public class Moon9QuestNodes : MonoBehaviour
    {
        [Header("Moon 9 Quest Configuration")]
        ...
        void SpawnQuestNodes()
        {
            for (int i = 0; i < questStartNodeCount; i++)
                CreateQuestNode($"QuestStart_Moon9_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestStart", $"moon9_quest_{i}");
            for (int i = 0; i < questCompletionNodeCount; i++)
                CreateQuestNode($"QuestComplete_Moon9_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestComplete", $"moon9_quest_{i}");
            for (int i = 0; i < objectiveMarkerCount; i++)
                CreateQuestNode($"Objective_Moon9_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "Objective", $"moon9_objective_{i}");
            Debug.Log($"🎯 Moon9QuestNodes spawned {questNodes.Count} quest nodes");
        }
