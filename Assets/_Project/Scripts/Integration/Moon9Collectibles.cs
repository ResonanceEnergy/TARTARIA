...
#pragma warning disable CS0414 // Placeholder counts for planned features
{
    ...
    public class Moon9Collectibles : MonoBehaviour
    {
        [Header("Moon 9: Corruption Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon9_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(0.6f, 0.3f, 0.6f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"BlightedRelic_Moon9_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "BlightedRelic", 15f, new Color(0.5f, 0.2f, 0.5f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon9_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(0.5f, 0.2f, 0.5f));
            Debug.Log($"💎 Moon9Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon9_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Corruption Aether Fragment", "BlightedRelic" => "Ancient Corruption Relic", "HiddenCache" => "Hidden CorruptedCache", _ => "Unknown Item" };
