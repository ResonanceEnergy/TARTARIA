...
#pragma warning disable CS0414 // Placeholder counts for planned features
    ...
    public class Moon5QuestNodes : MonoBehaviour
    {
        [Header("Moon 5: Ice Quest Configuration")]
        ...
        void SpawnQuestNodes()
        {
            for (int i = 0; i < questStartNodeCount; i++)
                CreateQuestNode($"QuestStart_Moon5_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestStart", $"moon5_quest_{i}");
            for (int i = 0; i < questCompletionNodeCount; i++)
                CreateQuestNode($"QuestComplete_Moon5_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestComplete", $"moon5_quest_{i}");
            for (int i = 0; i < objectiveMarkerCount; i++)
                CreateQuestNode($"Objective_Moon5_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "Objective", $"moon5_objective_{i}");
            Debug.Log($"🎯 Moon5QuestNodes spawned {questNodes.Count} quest nodes");
        }
