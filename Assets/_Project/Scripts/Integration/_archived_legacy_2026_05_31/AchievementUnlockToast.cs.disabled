...
namespace Tartaria.Integration  // NOTE: Lives in UI folder but part of Integration assembly (wires AchievementSystem→UI)
{
    /// <summary>
    ...
    public class AchievementUnlockToast : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            var achMgr = AchievementSystem.Instance;
            if (achMgr != null)
            {
                achMgr.OnAchievementUnlocked += OnAchievementUnlockedHandler;
            }
        ...
        void OnAchievementUnlockedHandler(AchievementSystem.AchievementDef achievement)
        {
            if (achievement == null) return;

            Debug.Log($"[AchievementToast] Unlocked: {achievement.title}");
            ShowToast(achievement.title, achievement.description);
        }

        void ShowToast(string title, string description, Sprite icon = null)
        {
