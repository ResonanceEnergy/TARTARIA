...
#pragma warning disable CS0414 // Placeholder counts for planned features
{
    ...
    public class Moon12Collectibles : MonoBehaviour
    {
        [Header("Moon 12: Shadow Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon12_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(0.3f, 0.3f, 0.4f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"UmbralRelic_Moon12_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "UmbralRelic", 15f, new Color(0.2f, 0.2f, 0.3f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon12_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(0.2f, 0.2f, 0.3f));
            Debug.Log($"💎 Moon12Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon12_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Shadow Aether Fragment", "UmbralRelic" => "Ancient Shadow Relic", "HiddenCache" => "Hidden DarknessCache", _ => "Unknown Item" };
