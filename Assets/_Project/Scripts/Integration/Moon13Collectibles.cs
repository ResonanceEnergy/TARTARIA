...
#pragma warning disable CS0414 // Placeholder counts for planned features
    ...
    public class Moon13Collectibles : MonoBehaviour
    {
        [Header("Moon 13: Convergence Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon13_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(1f, 0.8f, 1f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"TributeRelic_Moon13_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "TributeRelic", 15f, new Color(0.9f, 0.7f, 1f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon13_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(0.9f, 0.7f, 1f));
            Debug.Log($"💎 Moon13Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon13_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Convergence Aether Fragment", "TributeRelic" => "Ancient Convergence Relic", "HiddenCache" => "Hidden AetherCache", _ => "Unknown Item" };
