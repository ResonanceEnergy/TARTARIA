...
namespace Tartaria.Integration
{
    ...
    /// </summary>
    public class BossEncounterSystem : MonoBehaviour, ISaveDataProvider
    {
        public static BossEncounterSystem Instance { get; private set; }

        // 2026-06-03 HAMMER R4: URP-safe color set for GameObject.CreatePrimitive bodies.
        // Default URP/Lit ships with Standard shader → magenta. Replace shader + use _BaseColor.
        static void ApplyURPLitColor(GameObject body, Color c)
        {
            if (body == null) return;
            ...
            {
                // Built-in RP fallback — at least set Color so we're not magenta
        ...
        // ─── Persistence ───
        HashSet<string> _defeatedBosses = new HashSet<string>(); // Tracks defeated boss names
        Dictionary<string, BossRecord> _bossRecords = new Dictionary<string, BossRecord>(); // Best times/scores per boss
        ...
        public float HealthFraction => BossHPNormalized; // Alias for IntegrationBridge
        public string DisplayName => _currentBoss?.bossName ?? ""; // For IntegrationBridge boss health bar
        ...
        public bool IsBossDefeated(string bossName) => _defeatedBosses.Contains(bossName);
        public BossRecord GetBossRecord(string bossName) => _bossRecords.TryGetValue(bossName, out var record) ? record : null;
        ...
        void Awake()
        {
            ...
            _defeatedBosses = new HashSet<string>();
            _bossRecords = new Dictionary<string, BossRecord>();
        ...
        void SpawnOrUpdateBossVisuals(string phaseName)
        {
            ...
            if (name.Contains("mud") || name.Contains("colossus"))
            {
                if (_colossusVisualProxy == null)
                {
                    ...
                    ApplyURPLitColor(body, new Color(0.35f, 0.28f, 0.18f, 1f)); // deep mud (URP-safe)
            ...
            else if (name.Contains("rail"))
            {
                if (_railWraithVisual == null)
                {
                    ...
                    ApplyURPLitColor(railBody, new Color(0.25f, 0.22f, 0.3f));
            ...
            else if (name.Contains("sludge"))
            {
                if (_sludgeVisual == null)
                {
                    ...
                    ApplyURPLitColor(sludgeBody, new Color(0.18f, 0.32f, 0.22f));
            ...
            else if (name.Contains("sky") || name.Contains("reaver"))
            {
                if (_skyReaverVisual == null)
                {
                    ...
                    ApplyURPLitColor(reaverBody, new Color(0.15f, 0.18f, 0.35f));
            ...
            else if (IsFrequencyWraith() || name.Contains("frequency"))
            {
                if (_frequencyWraithVisual == null)
                {
                    ...
                    ApplyURPLitColor(wraithBody, new Color(0.6f, 0.4f, 0.85f)); // shifting violet
        ...
        void DefeatBoss()
        {
            ...
            // Track defeat
            _defeatedBosses.Add(_currentBoss.bossName);

            // Update record if better
            if (!_bossRecords.TryGetValue(_currentBoss.bossName, out var existingRecord) || _encounterTime < existingRecord.bestTime)
            {
                _bossRecords[_currentBoss.bossName] = new BossRecord
                {
                    bossName = _currentBoss.bossName,
                    bestTime = _encounterTime,
                    bestScore = result.performanceScore,
                    noHitCleared = result.noHitClear,
                    defeatCount = existingRecord != null ? existingRecord.defeatCount + 1 : 1
                };
            }
            else if (existingRecord != null)
            {
                // Update defeat count even if not better time
                existingRecord.defeatCount++;
                _bossRecords[_currentBoss.bossName] = existingRecord;
            }

            // Spawn tiered loot based on player level
        ...
        public string GetProviderKey() => "BossEncounterSystem";

        public object GetSaveData()
        {
            var records = new List<BossRecordData>();
            foreach (var kvp in _bossRecords)
            {
                records.Add(new BossRecordData
                {
                    bossName = kvp.Value.bossName,
                    bestTime = kvp.Value.bestTime,
                    bestScore = kvp.Value.bestScore,
                    noHitCleared = kvp.Value.noHitCleared,
                    defeatCount = kvp.Value.defeatCount
                });
            }

            return new BossEncounterSaveData
            {
                defeatedBosses = new List<string>(_defeatedBosses).ToArray(),
                bossRecords = records.ToArray()
            };
        }
        ...
        public void RestoreSaveData(object data)
        {
            ...
            {
                Debug.LogWarning("[BossEncounterSystem] RestoreSaveData expected JSON string");
                return;
            }

            var saveData = JsonUtility.FromJson<BossEncounterSaveData>(json);
            if (saveData == null) return;

            _defeatedBosses.Clear();
            _bossRecords.Clear();

            if (saveData.defeatedBosses != null)
            {
                foreach (var bossName in saveData.defeatedBosses)
                {
                    _defeatedBosses.Add(bossName);
                }
            }

            if (saveData.bossRecords != null)
            {
                foreach (var recordData in saveData.bossRecords)
                {
                    _bossRecords[recordData.bossName] = new BossRecord
                    {
                        bossName = recordData.bossName,
                        bestTime = recordData.bestTime,
                        bestScore = recordData.bestScore,
                        noHitCleared = recordData.noHitCleared,
                        defeatCount = recordData.defeatCount
                    };
                }
            }

            Debug.Log($"[BossEncounterSystem] Restored {_defeatedBosses.Count} defeated bosses, {_bossRecords.Count} records");
        }
    }
    ...
    /// <summary>
    /// Boss record for persistence - best time and score per boss.
    /// </summary>
    public class BossRecord
    {
        ...
        public bool noHitCleared;
        public int defeatCount;
    }

    /// <summary>
    /// Serializable save data for BossEncounterSystem.
    /// </summary>
    [Serializable]
    public class BossEncounterSaveData
    {
        public string[] defeatedBosses;
        public BossRecordData[] bossRecords;
    }

    [Serializable]
    public class BossRecordData
    {
        public string bossName;
        public float bestTime;
        public float bestScore;
