...
#pragma warning disable CS0414 // Placeholder counts for planned features
    ...
    public class Moon6Collectibles : MonoBehaviour
    {
        [Header("Moon 6: Lava Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon6_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(1f, 0.5f, 0.2f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"EmberRelic_Moon6_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "EmberRelic", 15f, new Color(1f, 0.4f, 0.2f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon6_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(1f, 0.4f, 0.2f));
            Debug.Log($"💎 Moon6Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon6_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Lava Aether Fragment", "EmberRelic" => "Ancient Lava Relic", "HiddenCache" => "Hidden VolcanicCache", _ => "Unknown Item" };
