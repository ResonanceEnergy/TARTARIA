...
#pragma warning disable CS0414 // Placeholder counts for planned features
    ...
    public class Moon11QuestNodes : MonoBehaviour
    {
        [Header("Moon 11 Quest Configuration")]
        ...
        void SpawnQuestNodes()
        {
            for (int i = 0; i < questStartNodeCount; i++)
                CreateQuestNode($"QuestStart_Moon11_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestStart", $"moon11_quest_{i}");
            for (int i = 0; i < questCompletionNodeCount; i++)
                CreateQuestNode($"QuestComplete_Moon11_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestComplete", $"moon11_quest_{i}");
            for (int i = 0; i < objectiveMarkerCount; i++)
                CreateQuestNode($"Objective_Moon11_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "Objective", $"moon11_objective_{i}");
            Debug.Log($"🎯 Moon11QuestNodes spawned {questNodes.Count} quest nodes");
        }
