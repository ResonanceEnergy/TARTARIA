...
#pragma warning disable CS0414 // Placeholder counts for planned features
    ...
    public class Moon10QuestNodes : MonoBehaviour
    {
        [Header("Moon 10 Quest Configuration")]
        ...
        void SpawnQuestNodes()
        {
            for (int i = 0; i < questStartNodeCount; i++)
                CreateQuestNode($"QuestStart_Moon10_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestStart", $"moon10_quest_{i}");
            for (int i = 0; i < questCompletionNodeCount; i++)
                CreateQuestNode($"QuestComplete_Moon10_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "QuestComplete", $"moon10_quest_{i}");
            for (int i = 0; i < objectiveMarkerCount; i++)
                CreateQuestNode($"Objective_Moon10_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), "Objective", $"moon10_objective_{i}");
            Debug.Log($"🎯 Moon10QuestNodes spawned {questNodes.Count} quest nodes");
        }
