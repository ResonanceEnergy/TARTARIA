...
namespace Tartaria.Integration
{
    ...
    {
        [Header("Moon 6: Lava Interactive Objects")]
        ...
        void SpawnInteractiveObjects()
        {
            for (int i = 0; i < doorCount; i++)
                CreateDoor($"MagmaDoor_{i}", new Vector3(Random.Range(-60f, 60f), 0.5f, Random.Range(-60f, 60f)), i < 3, "Key_{i}");
            ...
            Debug.Log($"🔧 Moon6InteractiveObjects: {interactives.Count} objects spawned");
        }
        GameObject CreateDoor(string name, Vector3 pos, bool locked, string key) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Cube); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(3f, 4f, 0.3f); var io = obj.AddComponent<Moon6InteractableObject>(); io.interactType = "door"; io.isLocked = locked; io.requiredKey = key; interactives.Add(obj); return obj; }
        GameObject CreateLever(string name, Vector3 pos, string targetId) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Cylinder); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(0.3f, 1.5f, 0.3f); var io = obj.AddComponent<Moon6InteractableObject>(); io.interactType = "lever"; io.targetId = targetId; interactives.Add(obj); return obj; }
        GameObject CreatePressurePlate(string name, Vector3 pos, float weight) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Cylinder); obj.name = name; obj.transform.position = pos; obj.transform.localScale = new Vector3(2f, 0.1f, 2f); var io = obj.AddComponent<Moon6InteractableObject>(); io.interactType = "pressurePlate"; io.requiredWeight = weight; interactives.Add(obj); return obj; }
        GameObject CreatePuzzleElement(string name, Vector3 pos, string puzzleType) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Sphere); obj.name = name; obj.transform.position = pos; var io = obj.AddComponent<Moon6InteractableObject>(); io.interactType = "puzzle"; io.puzzleType = puzzleType; interactives.Add(obj); return obj; }
        GameObject CreateBreakable(string name, Vector3 pos, float health) { GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Cube); obj.name = name; obj.transform.position = pos; var io = obj.AddComponent<Moon6InteractableObject>(); io.interactType = "breakable"; io.health = health; interactives.Add(obj); return obj; }
        void OnDestroy() { foreach (var obj in interactives) if (obj != null) Destroy(obj); interactives.Clear(); }
    }
    public class Moon6InteractableObject : MonoBehaviour, IInteractable
    {
