...
namespace Tartaria.Integration
{
    ...
    public class Moon7Collectibles : MonoBehaviour
    {
        [Header("Moon 7: Underwater Collectibles")]
        ...
        void SpawnCollectibles()
        {
            for (int i = 0; i < aetherFragmentCount; i++)
                CreateCollectible($"AetherFragment_Moon7_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 3f), Random.Range(-65f, 65f)), "AetherFragment", 5f, new Color(0.3f, 0.7f, 1f));
            for (int i = 0; i < ancientRelicCount; i++)
                CreateCollectible($"AbyssalRelic_Moon7_{i}", new Vector3(Random.Range(-65f, 65f), Random.Range(0.5f, 2f), Random.Range(-65f, 65f)), "AbyssalRelic", 15f, new Color(0.2f, 0.6f, 0.9f));
            for (int i = 0; i < hiddenCacheCount; i++)
                CreateCollectible($"HiddenCache_Moon7_{i}", new Vector3(Random.Range(-70f, 70f), Random.Range(0.5f, 4f), Random.Range(-70f, 70f)), "HiddenCache", 30f, new Color(0.2f, 0.6f, 0.9f));
            Debug.Log($"💎 Moon7Collectibles spawned {collectibles.Count} collectibles");
        }
        GameObject CreateCollectible(string name, Vector3 position, string collectibleType, float rsReward, Color glowColor)
        {
            ...
            item.itemId = $"moon7_{collectibleType.ToLower()}_{name}";
            item.collectibleType = collectibleType;
            item.rsReward = rsReward;
            item.displayName = collectibleType switch { "AetherFragment" => "Underwater Aether Fragment", "AbyssalRelic" => "Ancient Underwater Relic", "HiddenCache" => "Hidden DeepSeaCache", _ => "Unknown Item" };
