...
using SkillId = Tartaria.Core.Enums.SkillId;
using QuestObjectiveType = Tartaria.Core.QuestObjectiveType;
...
namespace Tartaria.Integration
{
    ...
    public class GameLoopController : ECSMonoBehaviour, IGameLoopService
    {
        ...
        {
            if (_em == null || !_em.Exists(_rsEntity)) return;
        ...
        void PollResonanceScore()
        {
            ...
            if (_em == null || !_em.Exists(_rsEntity)) { _ecsReady = false; return; }
        ...
        void HandleClimaxCompleted(int moonIndex)
        {
            ...
            NotificationSystem.Instance?.ShowNotification("Moon climax complete!", NotificationType.Achievement);
        ...
        void HandleWaveCleared(int waveIndex)
        {
            ...
            NotificationSystem.Instance?.ShowNotification($"Wave {waveIndex + 1} cleared!", NotificationType.Combat);
        ...
        void HandleAllWavesCleared()
        {
            ...
            GameEvents.RaiseHUDHideBossHealth();
            NotificationSystem.Instance?.ShowNotification("All waves eliminated! Victory!", NotificationType.Combat);
            AdaptiveMusicController.Instance?.ExitCombat();
            SaveManager.Instance?.MarkDirty();
        }
        ...
        void HandleAchievementUnlocked(AchievementSystem.AchievementDef def)
        {
            ...
            NotificationSystem.Instance?.ShowNotification($"Achievement: {def.title}", NotificationType.Achievement);
        ...
        {
            NotificationSystem.Instance?.ShowNotification($"Trust change: {companionId} → {Mathf.RoundToInt(newTrust)}", NotificationType.Info);
        }
        ...
        /// <summary>
        /// Get current Resonance Score for prerequisite checks.
        /// </summary>
        public float GetCurrentRS()
        {
            if (!_ecsReady || !_em.Exists(_rsEntity)) return 0f;
            return _em.GetComponentData<ResonanceScore>(_rsEntity).CurrentRS;
        }
        ...
        {
            if (playerInput == null || _em == null || !_em.Exists(_playerEntity)) return;
        ...
        void HandleLanguageChanged(SystemLanguage lang)
        {
