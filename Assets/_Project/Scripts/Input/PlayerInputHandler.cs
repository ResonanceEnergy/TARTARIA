...
#pragma warning disable CS0414 // Field assigned but never used - reserved for future implementation
    ...
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        CameraController _cachedCameraController;
        ...
        void Awake()
        {
            ...
            // 2026-06-03 DEBT #12: RunInBackgroundGuard (BeforeSceneLoad) owns the
            // canonical Application.runInBackground + InputSystem IgnoreFocus + GameViewInput
            // settings. Redundant assignments here removed.
        ...
        void Update()
        {
            ...
            // docs/15 §12 (C.L6): defense-in-depth — also gate on CameraController.IsCinematic
            // so a cinematic that fires GameEvents.OnCinematicStart without flipping GameState
            // still blocks player input. CameraController is a singleton-like scene component
            // (one per scene per CLAUDE.md camera bootstrap). Cache reference lazily.
            if (_cachedCameraController == null)
                _cachedCameraController = FindFirstObjectByType<CameraController>();
            if (_cachedCameraController != null && _cachedCameraController.IsCinematic) return;
        ...
        void HandleMovementInput()
        {
            ...
            // Face movement direction (only when actually moving and a horizontal direction exists).
            // Gated by null/zero guards so existing rotation behavior is preserved when MoveDirection==0.
            ...
            // Drive Animator Speed parameter (normalized 0..1 for Locomotion BlendTree).
            // Null guard: Player.prefab may ship without an Animator on capsule builds; do nothing then.
            if (_anim != null && _anim.runtimeAnimatorController != null)
            {
                float maxSpeed = moveSpeed * sprintMultiplier;
                float currentMagnitude = maxSpeed > 0.001f ? Mathf.Clamp01(MoveDirection.magnitude * (speed / maxSpeed)) : 0f;
                _anim.SetFloat(_animSpeedHash, currentMagnitude, 0.1f, Time.deltaTime);
            }
