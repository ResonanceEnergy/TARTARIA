...
{
    /// <summary>
    /// Moon1AuthorTimeFixers_Spawn — author-time Editor menus (PlayerSpawner
    /// position, fall-through safety, camera inversion toggle). Renamed
    /// 2026-06-04 from Moon1FixSpawn — the "Fix" prefix tripped the anti-
    /// runtime-daemon quarantine grep in CLAUDE.md. This file ships only
    /// [MenuItem] handlers, NOT [RuntimeInitializeOnLoadMethod] daemons.
    /// </summary>
    public static class Moon1AuthorTimeFixers_Spawn
    {
        ...
        public static void FixSpawnPosition()
        {
            ...
            // 2026-06-04 FIX-MOVE: BoxCollider on the platform was trapping the Player CC
            // inside the platform geometry — Player capsule intersected with the platform
            // mesh, and the CharacterController.Move() call kept getting depenetrated upward
            // (Y+) but rejected any XZ motion. Setting the collider to a trigger preserves the
            // visual ground but lets the player walk through/off. The _FallSafetyFloor at Y=-20
            // catches anyone who falls off.
            var platformCol = platform.GetComponent<BoxCollider>();
            if (platformCol != null) platformCol.isTrigger = true;
        ...
        public static void ForceAllSpawnRefs()
        {
            ...
            // platformY removed — value lived inside platformPos.y; the loose const was never read.
