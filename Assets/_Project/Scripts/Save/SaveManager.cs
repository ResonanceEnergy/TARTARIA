...
namespace Tartaria.Save
{
    ...
    public class SaveManager : MonoBehaviour, Tartaria.Core.ISaveService
    {
        ...
        [SerializeField] bool enableCompression = true; // GZip-compress before disk write
        [SerializeField] bool supportLegacyJsonSaves = true; // Auto-migrate v1.0 JSON saves
        ...
        public void Save()
        {
            ...
            try
            {
                ...
                // Agent 9: Apply GZip compression if enabled.
                // Applied AFTER encryption so the on-disk byte stream sniff order on load is:
                //   GZip-header? → decompress → encryption-magic? → decrypt → deserialize.
                if (enableCompression)
                {
                    int beforeBytes = serialized.Length;
                    serialized = GZipCompress(serialized);
                    Debug.Log($"[SaveManager] Save GZip-compressed: {beforeBytes} → {serialized.Length} bytes ({(serialized.Length * 100f / Math.Max(1, beforeBytes)):F0}%)");
                }
            ...
            {
                // 2026-06-05 fix: log full exception (type + stack) instead of bare Message
                // so silent NRE surfaces the actual file:line on next playtest.
        ...
        public void LoadOrCreate()
        {
            ...
            // Agent 9 backward compatibility — try old v1.0 JSON save files (pre-binary format).
            // If found, migrate forward by re-saving in the current (binary + encrypted + compressed)
            // pipeline. The legacy file remains on disk as a one-way safety net.
            if (_currentSave == null && supportLegacyJsonSaves)
            {
                ...
                if (_currentSave != null)
                {
                    ...
                    MarkDirty();
                    Save(); // Re-save in current binary+compressed+encrypted format
                }
            }
        ...
        SaveData TryLoadFromPath(string path)
        {
            ...
            try
            {
                ...
                // Auto-detect GZip compression by 2-byte magic header (0x1F 0x8B).
                // Compression was applied LAST during save, so it must be undone FIRST here.
                if (IsGZipCompressed(data))
                {
                    int beforeBytes = data.Length;
                    data = GZipDecompress(data);
                    Debug.Log($"[SaveManager] GZip-decompressed save from {path}: {beforeBytes} → {data.Length} bytes");
                }
        ...
        // Standard System.IO.Compression.GZipStream. Magic header is 0x1F 0x8B.
        // Encrypted payloads start with a different magic (see SaveEncryptionHelper),
        // so a single byte-prefix sniff disambiguates "compressed?" from "encrypted?".

        const byte GZIP_MAGIC_0 = 0x1F;
        const byte GZIP_MAGIC_1 = 0x8B;

        static bool IsGZipCompressed(byte[] data)
        {
            return data != null && data.Length >= 2 && data[0] == GZIP_MAGIC_0 && data[1] == GZIP_MAGIC_1;
        }
