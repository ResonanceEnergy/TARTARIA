...
namespace Tartaria.Core
{
    ...
    public class GameBootstrap : MonoBehaviour
    {
        ...
        /// <summary>
        /// Direct-Play dev workflow: when Echohaven_VerticalSlice is the active scene
        /// at AfterSceneLoad (skipping the MainMenu → Start-click flow), GameStateManager
        /// stays in Boot forever, which gates PlayerInputHandler.Update via the IsPlaying
        /// guard. Flip to Exploration once, only if no other system has advanced state yet.
        ///
        /// Legitimate boot-flow path: replaces the deleted Moon1AutoEnterExploration daemon.
        /// MainMenu → BeginGameplay → TriggerSceneLoad still drives Boot → Loading → Exploration
        /// via SceneLoader, so this never fires in that path (CurrentState != Boot by then).
        /// </summary>
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
        static void EnsureExplorationStateForDirectPlay()
        {
            ...
            if (mgr == null) return;
            if (mgr.CurrentState != GameState.Boot) return; // MainMenu / SceneLoader already advanced

            Debug.Log("[GameBootstrap] Direct-Play on Echohaven_VerticalSlice detected — state==Boot, transitioning to Exploration so PlayerInputHandler.Update is ungated.");
