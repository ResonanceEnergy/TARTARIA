...
namespace Tartaria.Core
{
    ...
    public class PlayerSentimentTracker : MonoBehaviour
    {
        ...
        void RecordInputSpam()
        {
            ...
            // CrashReporter.AddBreadcrumb("INPUT_SPAM: Player mashing buttons (possible frustration)");
        ...
        void RecordLongIdle(float idleTime)
        {
            _longIdleCount++;
            // TODO: Restore after implementing event-driven telemetry (CrashReporter moved to LiveOps)
            // CrashReporter.AddBreadcrumb($"LONG_IDLE: {idleTime:F0}s idle (possible stuck/confused)");
        ...
        public static void RecordPlayerDeath(string questID = null)
        {
            ...
            if (!string.IsNullOrEmpty(questID))
            {
                if (_instance._lastFailedQuest == questID)
                {
                    ...
                    // TODO: Restore after implementing event-driven telemetry (CrashReporter moved to LiveOps)
                    // CrashReporter.AddBreadcrumb($"QUEST_RETRY: {questID} (attempt #{_instance._questRestartCount})");
                }
                else
                {
