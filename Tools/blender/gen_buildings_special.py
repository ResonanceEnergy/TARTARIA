...
cyl("body", 1.0, 5.5, (0, 0, 2.75), stone, verts=18)
# Battlements at top
for i in range(8):
    a = i*(math.pi/4)
    cube(f"crenel_{i}", (math.cos(a)*1.05, math.sin(a)*1.05, 5.65), (0.16, 0.10, 0.20), stone)
# Roof beacon
cone("roof", 1.0, 0.10, 0.80, (0, 0, 6.20), roof_m, verts=16)
sphere("flame", 0.18, (0, 0, 6.20), flame, segs=10, rings=8)
# Door
cube("door", (0, -1.02, 1.0), (0.30, 0.04, 0.85), make_material("WT_door", (0.30, 0.18, 0.10), roughness=0.80))
...
cube("wall", (0, 0, 1.10), (1.6, 1.3, 1.0), plaster2)
# Visible beams
cube("beam_v_l", (-1.50, -1.20, 1.10), (0.06, 0.06, 1.0), beam)
cube("beam_v_r", ( 1.50, -1.20, 1.10), (0.06, 0.06, 1.0), beam)
cube("beam_h", (0, -1.30, 2.10), (1.55, 0.06, 0.06), beam)
# Roof
cone("roof", 1.9, 0.05, 0.85, (0, 0, 2.50), roof_m3, verts=4)
cube("door", (0, -1.33, 0.95), (0.30, 0.03, 0.85), make_material("Ap_door", (0.30, 0.18, 0.10), roughness=0.80))
cube("win", (-0.50, -1.32, 1.55), (0.30, 0.02, 0.30), window)
cube("sign", (0.65, -1.45, 1.70), (0.30, 0.04, 0.20), sign_m)
# Hanging herb bundles
for i, x in enumerate([-0.40, 0.0, 0.40]):
    cube(f"bundle_{i}", (x, -1.20, 1.95), (0.06, 0.04, 0.20), herb)
