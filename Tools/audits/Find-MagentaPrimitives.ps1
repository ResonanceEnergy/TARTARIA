...
File=$file.Replace($Root+'\',''); Line=($i+1); Reason="primitive without  URP shader assignment" }
                }
            }
            if ($line -match '\.color\s*=' -and $line -match 
'\b(mat|material|rend\.material|sharedMaterial)\b') {
                $offenders += [pscustomobject]@{ 
File=$file.Replace($Root+'\',''); Line=($i+1); Reason="material.color  assignment (use SetColor _BaseColor)" }
            }
        }
    }

if ($offenders.Count -eq 0) {
    Write-Host "CLEAN - no magenta-risk primitive creations found" 
...
Write-Host ""
Write-Host "FOUND $($offenders.Count) magenta-risk sites" -ForegroundColor 
Red
exit 1
