...
| Anastasia companion | Rocker beat, dialogue, restoration support | Full |
| Lirael companion | Day-25 calendar gate, dialogue, lore triggers | Full |
| Cassian companion | Dialogue, narrative support | Full |
...
- Additional companions (Thorne), dialogue trees beyond Moon 1 NPCs
...
| Celestial | 528 Hz (Solfeggio MI / transformation) | White-gold | Descends from sky at peak RS | Particle rain, aurora wisps |
...
### Implementation Note — Accepted Deviation (2026-06-03)

The spec above prescribes a **GPU compute shader** for the per-voxel Aether simulation. The
shipped implementation uses a **Burst-compiled DOTS/ECS `ISystem`** instead:

- `Assets/_Project/Scripts/Core/AetherFieldSystem.cs` — `[BurstCompile] partial struct AetherFieldSystem : ISystem`
- `Assets/_Project/Scripts/Core/AetherFieldRenderBridge.cs` — copies the ECS node grid into a
  `RenderTexture3D` (Telluric→R, Harmonic→G, Celestial→B, total→A) for URP shader sampling
- `Assets/_Project/Scripts/Core/AetherFieldManager.cs` — Mono→ECS bridge (`RegisterSource`/`RegisterSink`/`GetBandIntensity`)

**Rationale (per `docs/best-practices/UNITY_6_PATTERNS.md` perf budget §2):**
1. The Burst job (with the 2026-06-02 source spatial pre-filter — 180 m cull around player)
   meets the 2.0 ms frame budget on the dev target. Profiler marker `Tartaria.AetherField.R6`
   confirms.
2. CPU-side authoring is interoperable with the rest of the ECS pipeline (`ResonanceScoreSystem`,
   `AetherCollector` queries) without a GPU↔CPU readback round-trip.
3. A GPU compute path remains the post-§16 "GATE 1 exit" stretch goal once Moons 2–13 ship and we
   re-evaluate against denser fountains/crystals (Moon 2/3). Until then, the deviation is
   load-bearing for shipping the 13 Moons (per the 2026-06-03 anti-circling mandate).

The Render Bridge already publishes the volume texture as `_AetherVolume` so any URP
RenderFeature swap to the compute path requires no shader-side rework.
...
| 528 Hz | Healing/restoration overlay, Celestial band, high RS states |
